...
class MainWindow(ctk.CTkFrame):
    def __init__(self, master_app):
        ...
        # --- ИЗМЕНЕНИЕ КОНФИГУРАЦИИ КОЛОНОК ---
        self.top_frame.grid_columnconfigure(0, weight=0)  # 0 = Версия фиксирована
        self.top_frame.grid_columnconfigure(1, weight=1)  # 1 = Обновление растягивается
        self.top_frame.grid_columnconfigure(2, weight=0)  # 0 = Кнопка настроек фиксирована

        # Version (ФИКСИРОВАННАЯ ШИРИНА)
        # Добавил width=110
        self.version_frame = ctk.CTkFrame(self.top_frame, width=110, height=ROW_HEIGHT, corner_radius=CORNER_RADIUS,
                                          fg_color=FRAME_BG, border_width=1, border_color=BORDER_COLOR)
        self.version_frame.grid(row=0, column=0, sticky="w", padx=(0, 5))  # sticky="w" чтобы держалось слева
        self.version_frame.pack_propagate(False)  # Важно: запрещаем сжиматься под контент
        ...
        self.pritunl_connect_btn = ctk.CTkButton(self.pritunl_buttons_frame, text="Connect", width=125,
                                                 height=ROW_HEIGHT, corner_radius=CORNER_RADIUS, font=f_btn,
                                                 command=self.master_app.on_pritunl_connect_click)
        self.pritunl_connect_btn.pack(side="left")

        self.pritunl_cancel_btn = ctk.CTkButton(self.pritunl_buttons_frame, text=tr("btn_cancel"), width=125,
                                                height=ROW_HEIGHT, corner_radius=CORNER_RADIUS,
                                                fg_color="#AA0000", hover_color="#880000", font=f_btn,
    ...
    def toggle_pritunl_ui(self, state):
        if state == 'working':
            self.pritunl_connect_btn.pack_forget()
            self.pritunl_cancel_btn.pack(side="left", padx=0)
        else:
            self.pritunl_cancel_btn.pack_forget()
            self.pritunl_connect_btn.pack(side="left", padx=0)
